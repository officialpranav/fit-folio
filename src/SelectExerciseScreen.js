...
export default function SelectExerciseScreen({ navigation, route, exercises=sampleResponse }) {
  return (
    <ScrollView style={{
      display: 'flex',
      ...
      <Button onPress={() => {navigation.navigate('CreateWorkoutScreen')}} >
        Back
      </Button>
      {exercises.map((exercise, key) => {return <ExerciseCard key={key} exercise={exercise} navigation={navigation} />})}
    </ScrollView>
  );
}

function ExerciseCard({exercise, navigation}){
  return (
    <Surface style={{
      margin: 6,
      padding: 12,
